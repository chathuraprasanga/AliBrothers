...
const SALE_SELECT = `
  SELECT s.id, s.sale_date as saleDate, s.customer_id as customerId, c.name as customerName,
         c.phone as customerPhone, c.address as customerAddress, s.vehicle_number as vehicleNumber,
         s.roll_count as rollCount, s.notes, s.created_at as createdAt, s.updated_at as updatedAt
  FROM sales s
...
export function deleteSale(id: number): void {
  ...
  const del = db.transaction(() => {
    ...
    db.prepare(`DELETE FROM stock_ledger WHERE reference_table = 'sales' AND reference_id = ?`).run(
      id
    )
  })
  del()
